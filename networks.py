...
def G_paper(
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    # Building blocks.
    def block(x, res, labels): # res = 2..resolution_log2
        label_layer = tf.reshape(labels, [-1, 7, 1, 1])
        net_label = upscale2d(label_layer, factor=2**res)
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res == 2: # 4x4
                ...
                with tf.variable_scope('Conv'):
                    ...
                    x = tf.concat([x, net_label], axis=1)
            else: # 8x8 and up
                if fused_scale:
    ...
    if structure == 'linear':
        x = block(combo_in, 2, labels_in)
        images_out = torgb(x, 2)
        for res in range(3, resolution_log2 + 1):
            lod = resolution_log2 - res
            x = block(x, res, labels_in)
            img = torgb(x, res)
    ...
    if structure == 'recursive':
        def grow(x, res, lod, labels_in):
            y = block(x, res, labels_in)
            img = lambda: upscale2d(torgb(y, res), 2**lod)
            if res > 2: img = cset(img, (lod_in > lod), lambda: upscale2d(lerp(torgb(y, res), upscale2d(torgb(x, res - 1)), lod_in - lod), 2**lod))
            if lod > 0: img = cset(img, (lod_in < lod), lambda: grow(y, res + 1, lod - 1, labels_in))
            return img()
        images_out = grow(combo_in, 2, resolution_log2 - 2, labels_in)
...
def D_paper(
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    if structure == 'linear':
        ...
        combo_out = block(x, 2, labels_in)

    # Recursive structure: complex but efficient.
    if structure == 'recursive':
        def grow(res, lod, labels_in):
            x = lambda: fromrgb(downscale2d(images_in, 2**lod), res)
            if lod > 0: x = cset(x, (lod_in < lod), lambda: grow(res + 1, lod - 1, labels_in))
            x = block(x(), res, labels_in); y = lambda: x
            if res > 2: y = cset(y, (lod_in > lod), lambda: lerp(x, fromrgb(downscale2d(images_in, 2**(lod+1)), res - 1), lod_in - lod))
            return y()
        combo_out = grow(2, resolution_log2 - 2, labels_in)
